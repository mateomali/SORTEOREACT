...
(() => {
  ...
  const renderTeamStats = (team) => {
    ...
    return [
      ...
      ['Velocidad', averageStat(team, 'rhythm')],
      ['Ida y vuelta', averageStat(team, 'stamina')],
      ['Pase/Vision', averageStat(team, 'pass_vision')],
  ...
  const renderTeamRadar = (summary) => {
    ...
    const points = teamRadarFields.map(([field], index) => {
      const value = Math.max(1, Math.min(6, averageField(field)));
      const point = pointFor(index, radius * (value / 6));
      return `<circle cx="${point.x.toFixed(1)}" cy="${point.y.toFixed(1)}" r="2.8" fill="#063d2b"><title>${escapeHtml(statLabels[field])}: ${value.toFixed(1)}</title></circle>`;
    }).join('');
    return `
      <div class="team-radar-card">
        <svg viewBox="0 0 ${size} ${size}" role="img" aria-label="Radar ${escapeHtml(summary.label)}">
          ${rings}
          ${axes}
  ...
  const issuesFor = (summaries) => {
    ...
    if (spread(summaries.map((team) => team.slow)) > 1) {
      ...
      issues.push(['media', `Jugadores lentos desparejos (${mostSlow.slow - leastSlow.slow} de diferencia)`, `${mostSlow.label} concentra ${mostSlow.slow} lentos y ${leastSlow.label} tiene ${leastSlow.slow}.`]);
    }
  ...
  const swapPlayersById = (leftId, rightId) => {
    ...
    const beforeSummaries = buildSummaries(currentSnapshots());
    ...
    const leftPlayer = playersById.get(String(leftId));
    const rightPlayer = playersById.get(String(rightId));
    manualComparisonBefore = {
      label: `${leftPlayer?.name || 'Jugador'} / ${rightPlayer?.name || 'Jugador'}`,
