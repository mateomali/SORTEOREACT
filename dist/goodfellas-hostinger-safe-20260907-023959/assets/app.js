(() => {
  ...
  const bindStaticTeamFormationDrag = () => {
    ...
    const cardStat = (card, field) => {
      ...
      if (field === 'PassVision' && !card.dataset[datasetKey]) return cardStat(card, 'Technique');
  ...
  const playerStatFields = ['technique', 'pass_vision', 'rhythm', 'stamina', 'defense_physical', 'attack', 'teamwork', 'mentality', 'regularity', 'goalkeeper_skill'];
  ...
  const initManualTeams = () => {
    ...
    let manualComparisonBefore = null;
    ...
    const manualComparisonMetrics = (summaries) => {
      if (!summaries?.length) return null;
      const averageSpread = (field) => spread(summaries.map((team) => (
        team.players.length ? (Number(team.statTotals[field] || 0) / team.players.length) : 0
      )));
      const lineSpread = Math.max(
        spread(summaries.map((team) => Number(team.pitchCounts.ARQ || 0))),
        spread(summaries.map((team) => Number(team.pitchCounts.DEF || 0))),
        spread(summaries.map((team) => Number(team.pitchCounts.MED || 0))),
        spread(summaries.map((team) => Number(team.pitchCounts.DEL || 0))),
      );
      return {
        total: spread(summaries.map((team) => team.total)),
        attack: averageSpread('attack'),
        pass_vision: averageSpread('pass_vision'),
        rhythm: averageSpread('rhythm'),
        stamina: averageSpread('stamina'),
        lines: lineSpread,
      };
    };

    const renderManualComparison = (afterSummaries) => {
      if (!manualComparisonBefore) return '';
      const before = manualComparisonBefore.metrics;
      const after = manualComparisonMetrics(afterSummaries);
      if (!before || !after) return '';
      const rows = manualComparisonFields.map(([field, label]) => {
        ...
        return { field, label, beforeValue, afterValue, delta, status };
      });
      const improved = rows.filter((row) => row.status === 'mejora').length;
      const worsened = rows.filter((row) => row.status === 'empeora').length;
      return `
        <div class="manual-comparison-panel">
          <div class="manual-comparison-head">
            <strong>Antes / despues del cambio</strong>
            <span class="${worsened > improved ? 'is-worse' : 'is-better'}">${escapeHtml(manualComparisonBefore.label || 'Cambio manual')}</span>
          </div>
          <div class="manual-comparison-grid">
            ${rows.map((row) => `
              <div class="manual-comparison-item is-${row.status}">
                <strong>${escapeHtml(row.label)}</strong>
                <span>${row.status === 'mejora' ? 'Mejora' : row.status === 'empeora' ? 'Empeora' : 'Igual'}</span>
                <small>Antes ${row.beforeValue.toFixed(1)} / Despues ${row.afterValue.toFixed(1)} / ${row.delta > 0 ? '+' : ''}${row.delta.toFixed(1)}</small>
              </div>
            `).join('')}
          </div>
        </div>
    ...
    const renderTeamRadar = (summary) => {
      const size = 188;
      const center = 94;
      const radius = 58;
      const labelRadius = 78;
      const pointFor = (index, valueRadius) => {
        const angle = (-Math.PI / 2) + ((Math.PI * 2 * index) / teamRadarFields.length);
        return {
          x: center + (Math.cos(angle) * valueRadius),
          y: center + (Math.sin(angle) * valueRadius),
        };
      };
      const averageField = (field) => summary.players.length
        ? (Number(summary.statTotals[field] || 0) / summary.players.length)
        : 1;
      const polygon = teamRadarFields.map(([field], index) => {
        const value = Math.max(1, Math.min(6, averageField(field)));
        const point = pointFor(index, radius * (value / 6));
        return `${point.x.toFixed(1)},${point.y.toFixed(1)}`;
      }).join(' ');
      const rings = [2, 4, 6].map((level) => {
        const points = teamRadarFields.map((_, index) => {
          const point = pointFor(index, radius * (level / 6));
          return `${point.x.toFixed(1)},${point.y.toFixed(1)}`;
        }).join(' ');
        return `<polygon points="${points}" fill="none" stroke="#d7e6df" stroke-width="1"></polygon>`;
      }).join('');
      const axes = teamRadarFields.map(([, label], index) => {
        const end = pointFor(index, radius);
        const labelPoint = pointFor(index, labelRadius);
        const anchor = Math.abs(labelPoint.x - center) < 7 ? 'middle' : (labelPoint.x > center ? 'start' : 'end');
        return `
          <g>
            <line x1="${center}" y1="${center}" x2="${end.x.toFixed(1)}" y2="${end.y.toFixed(1)}" stroke="#d7e6df" stroke-width="1"></line>
            <text x="${labelPoint.x.toFixed(1)}" y="${labelPoint.y.toFixed(1)}" text-anchor="${anchor}" dominant-baseline="middle" fill="#526b62" font-size="9" font-weight="800">${escapeHtml(label)}</text>
          </g>
        `;
      }).join('');
      const points = teamRadarFields.map(([field], index) => {
        const value = Math.max(1, Math.min(6, averageField(field)));
        const point = pointFor(index, radius * (value / 6));
        return `<circle cx="${point.x.toFixed(1)}" cy="${point.y.toFixed(1)}" r="2.8" fill="#063d2b"><title>${escapeHtml(analysisStatLabels[field])}: ${value.toFixed(1)}</title></circle>`;
      }).join('');
      return `
        <div class="team-radar-card">
          <svg viewBox="0 0 ${size} ${size}" role="img" aria-label="Radar ${escapeHtml(teamDisplayName(summary))}">
            ${rings}
            ${axes}
            <polygon points="${polygon}" fill="rgba(6,61,43,.18)" stroke="#063d2b" stroke-width="2"></polygon>
            ${points}
          </svg>
        </div>
      `;
    ...
    const describeAnalysisIssues = (summaries) => {
      ...
      ['attack', 'defense_physical', 'rhythm', 'stamina', 'pass_vision', 'technique', 'teamwork', 'mentality', 'regularity', 'goalkeeper_skill'].forEach((field) => {
    ...
    const renderManualAnalysis = () => {
      ...
      analysisPanel.innerHTML = `
        ...
        ${renderManualComparison(summaries)}
    ...
    const teamCharacteristics = (team) => {
      ...
      return {
        ...
        passVision: average('pass_vision'),
            ...
            return `
              <article class="team-characteristics-card ${color.className}">
                ...
                <div class="team-characteristics-stats">
                  ...
                  <span>Ida y vuelta ${summary.stamina.toFixed(1)}</span>
                  <span>Pase/Vision ${summary.passVision.toFixed(1)}</span>
    ...
    const assignPlayerToTeam = (key, targetTeam) => {
      ...
      const wasComplete = teamsAreComplete();
      const beforeSummaries = wasComplete ? buildSummariesFromSnapshots(teamSnapshots()) : null;
      ...
      if (wasComplete && beforeSummaries) {
        manualComparisonBefore = {
          label: player?.name || 'Cambio manual',
          metrics: manualComparisonMetrics(beforeSummaries),
        };
      }
      analysisVisible = wasComplete ? true : false;
