...
function repo_match_participants_basic(int $matchId): array
{
    $stmt = db()->prepare(
        'SELECT p.id, p.name, p.positions, p.pace, p.skill, p.photo_path, p.photo_position_x, p.photo_position_y, p.photo_zoom,
                p.technique, p.rhythm, p.stamina, p.defense_physical, p.attack, p.teamwork, p.mentality, p.regularity, p.goalkeeper_skill
         FROM match_players mp
         INNER JOIN players p ON p.id = mp.player_id
         WHERE mp.match_id = :mid
         ORDER BY p.name ASC'
...
function repo_player_result_adjustments(array $playerIds, int $excludeMatchId = 0): array
{
    ...
    $pdo = db();
    $playersJsonColumn = null;
    if (schema_column_exists($pdo, 'match_teams', 'formation_data')) {
        $playersJsonColumn = 'formation_data';
    } elseif (schema_column_exists($pdo, 'match_teams', 'players_json')) {
        $playersJsonColumn = 'players_json';
    }
    if ($playersJsonColumn === null) {
        return [];
    }

    $tracked = array_fill_keys($playerIds, true);
    $rows = $pdo->query(
        "SELECT mt.match_id, mt.team_number, mt.total_skill, mt.$playersJsonColumn AS players_json, mt.goals
         FROM match_teams mt
         INNER JOIN matches m ON m.id = mt.match_id
         WHERE m.status = 'finalizado'
           AND mt.$playersJsonColumn IS NOT NULL
           AND mt.$playersJsonColumn <> ''
