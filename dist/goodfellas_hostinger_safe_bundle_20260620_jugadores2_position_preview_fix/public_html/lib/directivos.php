...
function ensure_directivos_schema(): void
{
    ...
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS director_player_stat_votes (
            ...
            pass_vision TINYINT UNSIGNED NULL,
    ...
    if (!schema_column_exists($pdo, 'director_player_stat_votes', 'pass_vision')) {
...
{
    return ['attack', 'defense_physical', 'technique', 'pass_vision', 'rhythm', 'stamina', 'teamwork', 'mentality', 'regularity', 'goalkeeper_skill'];
}
    ...
    try {
        $stmt = $pdo->prepare(
            'INSERT INTO director_player_stat_votes
               (voter_id, player_id, attack, defense_physical, technique, pass_vision, rhythm, stamina, teamwork, mentality, regularity, goalkeeper_skill, comments, manually_modified)
             VALUES
               (:voter_id, :player_id, :attack, :defense_physical, :technique, :pass_vision, :rhythm, :stamina, :teamwork, :mentality, :regularity, :goalkeeper_skill, :comments, 1)
             ON DUPLICATE KEY UPDATE
               attack = VALUES(attack), defense_physical = VALUES(defense_physical), technique = VALUES(technique), pass_vision = VALUES(pass_vision),
