const goodfellasInitPlayers = () => {
  ...
  (() => {
    ...
    const statNames = ['technique', 'pass_vision', 'rhythm', 'stamina', 'defense_physical', 'attack', 'teamwork', 'mentality', 'regularity'];
    ...
    const radarLabels = {
      technique: 'Tecnica',
      pass_vision: 'Pase/Vision',
      rhythm: 'Velocidad',
      stamina: 'Ida y vuelta',
    ...
    const qualitativeStatText = (stat) => {
      ...
      if (stat.field === 'stamina') {
        return `${stat.label} ${quality}`;
      }
    ...
    const scoutStatRules = [
      ...
      },
      {
        field: 'pass_vision',
        label: 'Pase/Vision',
        strength: ['todavia juega con el parabrisas empañado', 've la simple y eso ya le evita varios incendios', 'encuentra al companero libre sin rifar la pelota', 'levanta la cabeza y cambia el ritmo con un pase', 'filtra pelotas que limpian lineas y hacen correr al rival', 'tiene mapa incluido: ve la jugada antes de que aparezca'],
        weakness: ['cuando tiene que elegir rapido, puede cerrar los ojos y darle para adelante', 'a veces tarda un segundo de mas y la ventana se cierra', 'si no tiene pase claro, puede abusar de la conduccion', 'no decide mal, pero puede animarse a romper mas lineas', 'con pelota suele leer bien el mapa', 'en lectura y pase esta sobrado: el problema suele ser alcanzarlo'],
      },
      {
        field: 'rhythm',
        label: 'Velocidad',
      ...
      },
      {
        field: 'stamina',
        label: 'Ida y vuelta',
        strength: ['aguanta el partido sin partirse en dos', 'puede repetir esfuerzos y seguir siendo util', 'cuando el partido se alarga, todavia tiene resto', 'sostiene presion, vuelta y marca sin apagarse'],
        weakness: ['si el ida y vuelta se estira, empieza a elegir esfuerzos', 'necesita administrar mejor el aire', 'cuando toca volver muchas veces, puede llegar justo', 'en partidos largos conviene no dejarlo solo para todo el recorrido'],
      },
      {
    ...
    const describeScoutPlayer = (player) => {
      ...
      const roleStrengthPriority = {
        arquero: ['goalkeeper_skill', 'regularity', 'mentality', 'teamwork', 'rhythm', 'defense_physical', 'technique', 'pass_vision'],
        defensor: ['defense_physical', 'stamina', 'rhythm', 'mentality', 'regularity', 'pass_vision', 'technique', 'teamwork', 'attack'],
        lateral: ['rhythm', 'defense_physical', 'stamina', 'pass_vision', 'teamwork', 'technique', 'regularity', 'attack', 'mentality'],
        mediocampista: ['pass_vision', 'teamwork', 'technique', 'rhythm', 'stamina', 'regularity', 'mentality', 'attack', 'defense_physical'],
        delantero: ['attack', 'rhythm', 'technique', 'pass_vision', 'mentality', 'regularity', 'teamwork', 'stamina', 'defense_physical'],
        comodin: ['regularity', 'teamwork', 'pass_vision', 'rhythm', 'technique', 'stamina', 'mentality', 'defense_physical', 'attack'],
      };
      const roleWeaknessPriority = {
        arquero: ['goalkeeper_skill', 'teamwork', 'mentality', 'technique', 'pass_vision', 'rhythm', 'defense_physical', 'regularity'],
        defensor: ['defense_physical', 'stamina', 'teamwork', 'pass_vision', 'technique', 'rhythm', 'mentality', 'regularity', 'attack'],
        lateral: ['rhythm', 'defense_physical', 'stamina', 'teamwork', 'pass_vision', 'technique', 'attack', 'regularity', 'mentality'],
        mediocampista: ['pass_vision', 'teamwork', 'technique', 'rhythm', 'stamina', 'mentality', 'regularity', 'defense_physical', 'attack'],
        delantero: ['attack', 'teamwork', 'pass_vision', 'rhythm', 'technique', 'mentality', 'regularity', 'stamina', 'defense_physical'],
        comodin: ['teamwork', 'regularity', 'pass_vision', 'technique', 'rhythm', 'stamina', 'mentality', 'defense_physical', 'attack'],
      };
      ...
      const contrastWeaknessPriority = (() => {
        if (role === 'defensor' && best.field === 'defense_physical') return ['attack', 'technique', 'pass_vision', 'teamwork', 'rhythm'];
        if (role === 'delantero' && best.field === 'attack') return ['teamwork', 'pass_vision', 'rhythm', 'technique', 'mentality'];
        if (role === 'mediocampista' && ['pass_vision', 'technique', 'teamwork'].includes(best.field)) return ['defense_physical', 'attack', 'rhythm', 'mentality'];
        if (role === 'arquero' && best.field === 'goalkeeper_skill') return ['teamwork', 'technique', 'pass_vision', 'rhythm', 'mentality'];
        return null;
      })();
      ...
      const qualitativeStatSentenceText = (stat) => {
        ...
        const article = stat.field === 'pass_vision'
          ? 'el'
          : (['Tecnica', 'Solidez', 'Mentalidad', 'Regularidad'].includes(stat.label) ? 'la' : 'el');
        return `${article} ${text}`;
      };
      const statAreaText = (stat) => ({
        technique: 'la tecnica',
        pass_vision: 'el pase y la vision',
        rhythm: 'el ritmo',
        stamina: 'el ida y vuelta',
      ...
      const fieldZones = {
        ...
        pass_vision: 'en pase y vision',
        rhythm: 'en el ritmo',
        stamina: 'en el ida y vuelta',
      ...
      const fieldUseLabels = {
        ...
        pass_vision: 'su pase y vision',
        rhythm: 'su ritmo',
        stamina: 'su ida y vuelta',
      ...
      const fieldRiskLabels = {
        ...
        pass_vision: 'cuando tiene que decidir rapido con pelota',
        rhythm: 'cuando el partido lo obliga a correr largo',
        stamina: 'cuando tiene que repetir esfuerzos',
      ...
      const statSemantics = {
        ...
        pass_vision: {
          high: ['encuentra pases que le dan aire al equipo', 'lee mejor que el promedio cuando conviene tocar y cuando acelerar', 'puede limpiar una jugada con una descarga simple y bien elegida'],
          max: ['el pase y la vision estan al maximo: ve ventajas antes de que el rival las cierre'],
          low: ['cuando tiene que decidir rapido, puede elegir tarde o forzado', 'si no tiene una opcion clara, puede cortar la circulacion del equipo'],
        },
        rhythm: {
        ...
        stamina: {
          high: ['puede repetir esfuerzos sin desaparecer', 'sostiene el recorrido cuando el partido pide aire', 'llega al cierre con resto para seguir ayudando'],
          max: ['el ida y vuelta es excelente: mantiene intensidad incluso cuando el partido se alarga'],
          low: ['cuando el partido exige ida y vuelta, puede empezar a administrar demasiado', 'si le cargan mucho recorrido, el rendimiento cae por cansancio'],
        },
        defense_physical: {
      ...
      const tacticalAdvice = {
        ...
        pass_vision: 'En el armado, conviene acercarle opciones claras para que use el pase sin forzar la jugada.',
        rhythm: 'En el armado, no conviene pedirle recorridos largos todo el tiempo: mejor apoyos cerca y movimientos cortos.',
        stamina: 'En el armado, conviene medirle los recorridos largos y no dejarlo sosteniendo toda la banda solo.',
