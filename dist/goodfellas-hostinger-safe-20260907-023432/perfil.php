...
$statHelp = [
    'technique' => 'Control, primer toque, gambeta y calidad individual con la pelota.',
    'pass_vision' => 'Pase, lectura, cambio de frente, asistencia y decision con pelota.',
    'rhythm' => 'Pique corto, aceleracion, velocidad para atacar, cerrar espacios y volver.',
    'stamina' => 'Capacidad de sostener el ida y vuelta: seguir volviendo, presionando y marcando durante todo el partido.',
    'defense_physical' => 'Marca, quite, anticipo, presion, fuerza y choque.',
...
function profile_scout_data_attrs(array $player): string
{
    $attrs = [
        ...
        'player-scout-pass-vision' => number_format(player_effective_stat($player, 'pass_vision'), 1, '.', ''),
        'player-scout-rhythm' => number_format(player_effective_stat($player, 'rhythm'), 1, '.', ''),
        'player-scout-stamina' => number_format(player_effective_stat($player, 'stamina'), 1, '.', ''),
    ...
    $stmt = db()->prepare(
        "SELECT p.*, mp.team_number, mp.assigned_position, mp.is_goalkeeper, mp.lineup_order, mp.formation_line_order
         ...
         ORDER BY mp.team_number ASC,
           FIELD(mp.assigned_position, 'ARQ', 'DEF', 'LAT', 'MED', 'DEL'),
...
function profile_position_base_rating(array $player, string $position): float
{
    ...
    if ($position === 'ARQ') {
        ...
        return player_apply_regularity_adjustment(
            ($goalkeeperSkill * 0.36)
            + (player_effective_stat($player, 'defense_physical') * 0.12)
            + (player_effective_stat($player, 'rhythm') * 0.08)
            + (player_effective_stat($player, 'stamina') * 0.08)
            + (player_effective_stat($player, 'technique') * 0.08)
            + (player_effective_stat($player, 'pass_vision') * 0.06)
            + (player_effective_stat($player, 'teamwork') * 0.12)
...
function profile_team_characteristics_summary(array $players): array
{
    ...
    return [
        ...
        'pass_vision' => $average('pass_vision'),
...
function profile_render_team_characteristics(array $players): string
{
    ...
    <div class="public-team-characteristics">
      ...
      <div class="team-characteristics-stats">
        ...
        <span>Velocidad <?= h(number_format((float) $summary['rhythm'], 1)) ?></span>
        <span>Ida y vuelta <?= h(number_format((float) $summary['stamina'], 1)) ?></span>
        <span>Pase/Vision <?= h(number_format((float) $summary['pass_vision'], 1)) ?></span>
              ...
              <?php foreach (player_pitch_lines() as $line): ?>
                ...
                <div class="formation-line<?= $line === 'DEF' && !empty($lines['LAT']) ? ' is-projected-defense' : '' ?>">
                  ...
                  <div class="line-players">
                    ...
                    <?php else: ?>
                      <?php foreach ($linePlayers as $teamPlayer): ?>
                        <?php
                          $assignedLine = strtoupper((string) ($teamPlayer['assigned_position'] ?? $line));
                          $isCurrent = (int) $teamPlayer['id'] === $currentPlayerId;
                          $formationOverall = player_overall_rating($teamPlayer);
                          $formationCardRating = profile_player_fifa_overall(profile_adjusted_position_rating($teamPlayer, $assignedLine));
                        ?>
                        <div
                          ...
                          data-player-pass-vision="<?= h(number_format(player_effective_stat($teamPlayer, 'pass_vision'), 1, '.', '')) ?>"
                          data-player-rhythm="<?= h(number_format(player_effective_stat($teamPlayer, 'rhythm'), 1, '.', '')) ?>"
                          data-player-stamina="<?= h(number_format(player_effective_stat($teamPlayer, 'stamina'), 1, '.', '')) ?>"
